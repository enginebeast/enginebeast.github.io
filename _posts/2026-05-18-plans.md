...
지금 당장은 자동차가 장애물을 인식하고 피하는 것은 나중 문제이다. 가장 먼제 해결할 문제는 미니 자율 주행 자동차가 시작점에서 목표 지점까지 어떻게 이동할 것인가이다. 그걸 위해서는 크게 3가지 질문이 필요하다고 결론 내렸다. 첫째, 자율 주행 자동차가 어디가 목표 지점인지 알 수 있는가? 둘째, 자율 주행 자동차가 목표 지점까지 가기 위한 동선을 파악할 수 있는가?

그걸 위해서는 당장 자동차가 자신이 존재하는 공간을 데카르트 평면 공간으로 인지하고 동선을 짜는 단계에서 시작해도 문제가 없다.

하지만 진짜 문제는 경로 이동 중 누적되는 위치 오차를 자동차가 어떻게 수정하게 할 것인가이다. 기존의 자율 주행 자동차는 gps를 통해 쉽게 이를 파악할 수 있지만 애초에 이동 거리가 너무 짧은 미니 자율 주행 자동차의 경우에는 이 방식을 이용할 수 없다. 당장, 떠오르는 방식은 표지판이나 QR 코드 같은 특정 사인을 이용하는 방식이 있다. 또한 그외에도 어떤 방식이 있을지 조사해 보고 실제로 적용해 보는 것을 다음 목표로 정하였다.